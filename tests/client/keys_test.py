...
@pytest.mark.vcr
def test_keys(client: Client) -> None:
    """Tests fetching of all keys"""
    keys = client.keys(
        PROJECT_ID, {"page": 2, "limit": 3, "disable_references": "1", "filter_archived": "exclude"}
    )
...
@pytest.mark.vcr
def test_keys_cursor(client: Client) -> None:
...
@pytest.mark.vcr
def test_create_keys(client: Client) -> None:
    ...
    keys = client.create_keys(
        ...
        [
            ...
            },
            {
                "key_name": "python_2",
                "platforms": ["web"],
                "translations": [{"language_iso": "en", "translation": "Hi from Python"}],
            },
        ],
    )
    assert keys.project_id == PROJECT_ID
...
@pytest.mark.vcr
def test_create_key(client: Client) -> None:
    ...
    keys = client.create_keys(
        PROJECT_ID,
        {
            "key_name": "python_3",
            "platforms": ["ios", "android"],
            "translations": [{"language_iso": "ru_RU", "translation": "Привет от Python"}],
        },
    )
    assert keys.project_id == PROJECT_ID
    key = keys.items[0]
...
@pytest.mark.vcr
def test_key(client: Client) -> None:
...
@pytest.mark.vcr
def test_update_key(client: Client) -> None:
    """Tests updating of a key"""
    key = client.update_key(
        PROJECT_ID, KEY_ID, {"description": "Updated by Python", "tags": ["python"]}
    )
    assert key.project_id == PROJECT_ID
    assert key.key_id == KEY_ID
    assert key.description == "Updated by Python"
...
@pytest.mark.vcr
def test_update_keys(client: Client) -> None:
    """Tests bulk key update"""
    keys = client.update_keys(
        PROJECT_ID,
        [
            {"key_id": 48855757, "description": "Bulk updated", "tags": ["bulk-python"]},
            {
                "key_id": 48855758,
                "translations": [
                    {"language_iso": "ru_RU", "translation": "Обновлённый перевод Python"}
                ],
            },
        ],
    )
...
@pytest.mark.vcr
def test_delete_key(client: Client) -> None:
...
@pytest.mark.vcr
def test_delete_keys(client: Client) -> None:
