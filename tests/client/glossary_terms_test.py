...
@pytest.mark.vcr
def test_glossary_terms(client):
...
@pytest.mark.vcr
def test_create_glossary_terms(client):
...
@pytest.mark.vcr
def test_update_glossary_terms(client):
    """Tests updating of glossary terms
    """
    new_term_desc = "A commonly used network device"

    glossary_terms = client.update_glossary_terms(PROJECT_ID, [
        {
            "id": GLOSSARY_TERM_ID,
            "description": new_term_desc,
            "caseSensitive": False
        }
    ])
...
@pytest.mark.vcr
def test_delete_glossary_terms(client):
